...
class segmentationUtils:
    ...
    def getROI(d, image):
        '''d = [x, y, width, height, centroidx, centroidy, distanceToCenter]'''
        dim = (128, 128)
        if len(d) > 0:
            ...
            mask = np.ones(16384).reshape(128, 128) * 127
            mask[int(mask.shape[0]/2 - crop_img.shape[0]/2) : int(mask.shape[0]/2 + crop_img.shape[0]/2), int(mask.shape[1]/2 - crop_img.shape[1]/2) : int(mask.shape[1]/2 + crop_img.shape[1]/2)] = crop_img
            ...
            image = cv.resize(image, dim, interpolation = cv.INTER_NEAREST)
            return image, image
