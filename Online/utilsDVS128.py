...
#     '''
#
#     def __init__(self, width, height, cam_device=0, m=6):
#         '''
#         Constructor
#         '''
...
#         self.colorPos = (0, 0, 255) # Positive events color (8-bits) -> WHITE
#         self.colorNeg = (255, 0, 0) # Negative events color (8-bits) -> BLACK
...
#         _, frame_3D = self.cap.read()
#         # original shape (480, 640, 3)
#         frame_2D_gray = cv.cvtColor(frame_3D, cv.COLOR_BGR2GRAY)
#         crop_frame = frame_2D_gray[:, 80:560]
#         crop_frame.shape
#         interp_frame = cv.resize(crop_frame, (128, 128), interpolation = cv.INTER_LANCZOS4)
#         return interp_frame
#
#
...
#         '''
#
#         # self.frame = np.flip(self.get_rgb_frame().T, 0)
#         # self.frame = eventsToFrame2(pol, x, y, self.get_rgb_frame()) # it calls the eventsToFrame function and get the event frame
...
#     return matrix
#
# def eventsToFrame2(pol, x, y, frame):
#     pol = np.array(pol) # Os eventos no array de Polaridade passam a ser -0.5 ou 0.5
#     for i in range(len(x)):
#         if pol[i] == 0:
#             frame[y[i], x[i]] = 0 # insere os eventos dentro da matriz de zeros
#         if pol[i] == 1:
#             frame[y[i], x[i]] = 255 # insere os eventos dentro da matriz de zeros
#     return np.flip(frame.T, 0)
#
